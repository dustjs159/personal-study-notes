...
* Application Layer(7 계층) : 응용 프로그램들이 속해있는 계층. HTTP, FTP 등의 프로콜을 사용하여 사용자들이 사용 가능한 프로그램들이 속한 계층
* Presentation Layer(6 계층) : 데이터들의 압축과 데이터들을 표시할 수 있는 문자열의 형태를 정의하는 계층. 계층간 서로 다른 문자 표현 방식을 알아보기 위해 문자 표현 방식을 정의하는 단계
* Session Layer(5 계층) : End Point간 세션 연결을 수립, 유지, 관리하는 계층. 세션의 상태를 점검하여 연결 상태의 불량 등을 확인
* Transport Layer(4 계층) : 송신자와 수신자 간 데이터 전송을 담당하는 계층. TCP는 데이터 송,수신에 있어서 신뢰성을 보장하기 위해 흐름제어, 혼잡제어 등을 수행. Port번호를 사용하여 서비스를 구분함
* Network Layer(3 계층) : 네트워크 상에서 수신자에게 패킷을 전송하기 위해 최적의 경로를 찾는 Routing을 진행. IP 주소를 기반으로 Routing을 함. 데이터 전송단위 : Packet
* Datalink Layer(2 계층) : 데이터를 수신하기 위한 장치의 MAC 주소를 기반으로 데이터의 목적지를 찾아 전송. 데이터 전송단위 : Frame
* Physical Layer(1 계층) : 정보를 전송하기 위한 물리적 매체를 정의하고 전기적 신호를 통해 데이터를 전송할 수 있는 물리적 경로를 설정. 데이터 전송단위 : Bit
